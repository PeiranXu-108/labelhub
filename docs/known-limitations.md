...
- Supervisor follow-up Tasks 11-19 document expanded requirements that are not implemented yet: richer task metadata/reward rules, JSONL/Excel import, drag-and-drop template building, rich media fields, dynamic form rules/layouts, field-level LLM assist, labeler navigation, multistage review, and production runtime verification.
...
- JSONL/Excel import, batch editing, and uploaded file/image materials are not production-ready until Tasks 12 and 14 are implemented and a retention/storage policy is decided.
